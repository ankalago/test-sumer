...
import { ITodoItem } from '../interfaces/components';
...
const TodoItem = ({ todoItem }: ITodoItem): JSX.Element => {
  ...
  const toggleStatus = (id: number, status: boolean) => {
    const itemsMapped = todos.map((i => ({ ...i, status: i.id === id ? status : i.status })))
    setTodos(itemsMapped)
  }

  const removeTodo = (id: number) => {
    const todoSelected = todos.find(i => i.id === id)
    setSelectedTodo(todoSelected?.id ?? 0)
    ...
    <li className="bg-white shadow overflow-hidden rounded-md px-6 py-4 flex justify-between gap-2">
      <div className="flex items-center">
        <Switch
          ...
          onChange={(checked) => toggleStatus(todoItem.id, checked)}
          className={classNames(
            todoItem.status ? 'bg-green-400' : 'bg-gray-200',
        ...
        >
          ...
          <span
            className={classNames(
              todoItem.status ? 'translate-x-5' : 'translate-x-0',
              'pointer-events-none relative inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200'
            )}
          >
                    <span
                      className={classNames(
                        todoItem.status ? 'opacity-0 ease-out duration-100' : 'opacity-100 ease-in duration-200',
                    ...
                    <span
                      className={classNames(
                        todoItem.status ? 'opacity-100 ease-in duration-200' : 'opacity-0 ease-out duration-100',
                        'absolute inset-0 h-full w-full flex items-center justify-center transition-opacity'
                      )}
                      aria-hidden="true"
                    >
      ...
      <div className={classNames(
        todoItem.status ? 'line-through text-green-400' : 'text-gray-500',
